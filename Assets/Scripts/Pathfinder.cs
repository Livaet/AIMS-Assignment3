...
public class PathFinder : MonoBehaviour
{
    ...
    public static List<Node> QuadraticBezier(List<Node> path)
    {
        List<Node> quadratic_bezier = new List<Node>();
        foreach (Node n in path)
        {
            quadratic_bezier.Add(n.copy());
        }

        for (int i=0; i < path.Count -2; i++)
        {
            if ((path[i].heading != path[i+1].heading) && (path[i+1].heading == path[i + 2].heading))//i+1 is a corner
            {
                Vector3 v0 = path[i].worldPosition;
                Vector3 v1 = path[i+1].worldPosition;
                Vector3 v2 = path[i+2].worldPosition;
                Vector3 position;
                float k;
                ...
                {
                    k = j / (3 - 1.0f);
                    position = (1.0f - k) * (1.0f - k) * v0 + 2.0f * (1.0f - k) * k * v1 + k * k * v2;

                    quadratic_bezier[i+j].worldPosition = position;
                }
                i += 3; // took care of this curve, go on to the next one
